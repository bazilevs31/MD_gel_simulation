...
class Atom(object):
    ...
    def __str__(self):
        if hasattr(self, 'type'):
            ...
            return "atom {0:5s}  m = {1:7.3f} neighs = {2:s}".format(self.name, self.m, str(self.neighbors))
...
class Mol():
    ...
    def __init__(self,topol='diamond',  n_ = None, npoly=2, scale=1.):
    ...
    @property
    def nparticles(self):
        return self.nmons + self.nnodes + self.ncions
    @property
    def nbonds(self):
...
Atoms
     """.format(natoms=nparticles, nbonds=nbond, xlo=0., ylo=0., zlo=0., xhi=mol.cell_dims[0], yhi=mol.cell_dims[1], zhi=mol.cell_dims[2], mass_nodes=mol.nodes[0].m, mass_mons=mol.nodes[0].m, mass_cions=mol.cions[0].m)
...
mol addrep 0
mol addrep 0
mol modselect 0 0 name is "1"
mol modselect 1 0 (name is "1" or name is "2")
mol modselect 2 0 name is "3"
# mol modselect 1 0 name is "2"
# mol modselect 1 0 all
#nodes
mol modcolor 0 0 ColorID 0
mol modstyle 0 0 VDW {r_vdw} 12.000000

#mons
mol modcolor 1 0 ColorID 1
mol modstyle 1 0 CPK {r_cpk} 0.100000 12.000000 12.000000

#cions
mol modcolor 2 0 ColorID 7
# mol modstyle 2 0 VDW 0.100000 2.000000
mol modstyle 2 0 Points {r_point}
mol modselect 1 0 name is 1 or name is 2
...
""".format(input_data=input_data, r_vdw=r_vdw, r_cpk=r_cpk, r_point=r_point)
...
def main():
    ...
    args = read_cmd_line()
    print args
    Gel = Mol(topol=args.lattice, scale=args.length_scale, npoly=args.nmonomers, n_=args.dimensions)
    print Gel
