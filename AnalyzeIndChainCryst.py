...
def analyze_chain_hist(u, args):
    ...
    for res in u.residues:
        ...
        resid_cryst = AnalyzeChain.get_chain_crystallinity(chords, threshold=threshold, neigh=neigh)
        # all_g2 += resid_cryst
...
def get_diff_chain_cryst(u, args):
    ...
    for i in range(Ntypes):
        cryst = 0.
        kk = 0
        # i think the error is here, I should get Nchains as the total number
        # of chains of this type
        # all good here
        #
        for iall in range(Nchains):
            if int(chainlengths[iall]) == int(chaintypes[i]):
                cryst += chaing2s[iall]
                kk += 1
        if kk == 0:
            cryst = 0.
